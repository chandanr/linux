	...
	xfs_extlen_t		len)
{
	...
	if (!xfs_verify_agbno(mp, sc->sa.agno, agbno) ||
	    !xfs_verify_agbno(mp, sc->sa.agno, agbno_next - 1))
		return;
	 ...
	 * detect aliasing.
	 */
	for (i = agbno; i < agbno_next; i++) {
		xfs_agblock_t	j;
		xfs_agblock_t	max_len;

		/*
		 * Max buffer size is the max remote xattr buffer size, which
		 * is one fs block larger than 64k.
		 */
		max_len = min_t(xfs_agblock_t, agbno_next - i,
				xfs_attr3_rmt_blocks(mp, XFS_XATTR_SIZE_MAX));

		for (j = i; j < max_len; j++) {
			bp = xfs_buf_incore(mp->m_ddev_targp,
					XFS_AGB_TO_DADDR(mp, sc->sa.agno, i),
					XFS_FSB_TO_BB(mp, j),
					XBF_TRYLOCK);
			if (!bp)
				continue;
	...
	bool			crosslinked)
{
	...
	fsbno = XFS_AGB_TO_FSB(sc->mp, sc->sa.agno, agbno);
	trace_xrep_dispose_btree_extent(sc->mp, sc->sa.agno, agbno, aglen);
	...
	void			*priv)
{
	...
	if (sc->ip != NULL) {
		 ...
		 */
		ASSERT(sc->sa.agno == NULLAGNUMBER);
		ASSERT(!sc->sa.pag);
